...
int HttpClient::startRequest(const char* aServerName, uint16_t aServerPort,
    ...
    int ret = sendInitialHeaders(aServerName, IPAddress(0, 0, 0, 0),
            aServerPort, aURLPath, aHttpMethod, aUserAgent);
...
int HttpClient::startRequest(const IPAddress& aServerAddress,
    ...
    // Now we're connected, send the first part of the request
    int ret = sendInitialHeaders(aServerName, aServerAddress, aServerPort,
            aURLPath, aHttpMethod, aUserAgent);
    if ((initialState == eIdle) && (HTTP_SUCCESS == ret)) {
        // This was a simple version of the API, so terminate the headers now
        finishHeaders();
    }
...
int HttpClient::sendInitialHeaders(const char* aServerName, IPAddress aServerIP,
    ...
    if (aServerName) {
        ...
        iClient->print(aServerName);
        if (aPort != kHttpPort && aPort != kHttpsPort) {
            iClient->print(':');
            iClient->print(aPort);
        }
        iClient->println();
...
int HttpClient::skipResponseHeaders() {
    // Whilst we haven't timed out & haven't reached the end of the headers
    while ((!endOfHeadersReached()) && available()) {
        (void) readHeader();
